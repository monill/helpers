...
function DownloadCurl($ch,$url, $folder, $headers, $id, $encoded = true, $appand = true, $trueFilename = ''){

    $ch = $ch;
    if ($trueFilename != '') {
        echo 'file : ' . $trueFilename;
    } else {
        echo 'file : ' . @end(explode('/', $url));
    }
    ...
    $my_file = 'arquivo.1.txt';
    $myfile = fopen($my_file, 'a') or die("Unable to open file! " . $my_file);
...
for ($i=$initialCount; $i < $FinalCount; $i++) {
    ...
    echo "$i: ";
    DownloadCurl($ch, $currentLink, 'teste', $headers, $i);
